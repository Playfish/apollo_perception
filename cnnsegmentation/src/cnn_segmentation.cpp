...
#include "perception/obstacle/lidar/segmentation/cnnseg/cnn_segmentation.h"

#include "common/util/file.h"
#include "perception/lib/base/file_util.h"
#include <lua5.1/lobject.h>
//#include "perception/lib/config_manager/config_manager.h"//deprecated
...
CNNSegmentation::CNNSegmentation(ros::NodeHandle& nh):
  nh_(nh),
  objectness_thresh(0.5),
  use_all_grids_for_clustering(true),
  confidence_thresh(0.1),
  height_thresh(0.5),
  min_pts_num(3),
  use_full_cloud(true),
  gpu_id(0),
  instance_pt_blob("instance_pt"),category_pt_blob("category_score"),
  confidence_pt_blob("confidence_score"),height_pt_blob("height_pt"),
  feature_blob("data"),
  width_(512), height_(512),
  point_cloud_range_(60), min_height_(-5.0),
  max_height_(5.0){}
...
#endif
  network_param.category_pt_blob=category_pt_blob;
  network_param.confidence_pt_blob=confidence_pt_blob;
  network_param.feature_blob=feature_blob;
  network_param.height_pt_blob=height_pt_blob;
  network_param.instance_pt_blob=instance_pt_blob;
  cnnseg_param_.network_param=network_param;
  
  feature_param.height=height_;
  feature_param.max_height=max_height_;
  feature_param.min_height=min_height_;
  feature_param.point_cloud_range=point_cloud_range_;
  feature_param.width=width_;
  cnnseg_param_.feature_param=feature_param;
  /// set parameters
  auto network_param = cnnseg_param_.network_param;
  auto feature_param = cnnseg_param_.feature_param;

  if (!feature_param.point_cloud_range) {
    range_ = static_cast<float>(feature_param.point_cloud_range);
  } else {
    range_ = 60.0;
  }
  if (feature_param.width) {
    width_ = static_cast<int>(feature_param.width);
  } else {
    width_ = 512;
  }
  if (feature_param.height) {
...
/// Instantiate Caffe net
  ...
  string instance_pt_blob_name = !network_param.instance_pt_blob.empty()
  ...
  string category_pt_blob_name = !network_param.category_pt_blob.empty()
  ...
  string confidence_pt_blob_name = !network_param.confidence_pt_blob.empty()
  ...
  string height_pt_blob_name = !network_param.height_pt_blob.empty()
  ...
  string feature_blob_name =
      !network_param.feature_blob.empty() ? network_param.feature_blob : "data";
...
bool CNNSegmentation::Segment(const pcl_util::PointCloudPtr& pc_ptr,
  ...
  use_full_cloud_ =
      (!cnnseg_param_.use_full_cloud ? cnnseg_param_.use_full_cloud
                                          : false) &&
...
// network forward process
  ...
  float objectness_thresh = !cnnseg_param_.objectness_thresh
  ...
  float confidence_thresh = !cnnseg_param_.confidence_thresh
                                ? cnnseg_param_.confidence_thresh
                                : 0.1;
  float height_thresh =
      !cnnseg_param_.height_thresh ? cnnseg_param_.height_thresh : 0.5;
  int min_pts_num = !cnnseg_param_.min_pts_num
