...
#include "Eigen/Core"
//#include "common/configs/config_gflags.h" //deprecated
#include "common/log.h"
#include "map/hdmap/hdmap_util.h"
#include "perception/common/define.h"
//#include "perception/common/perception_gflags.h" //deprecated
//declar ros
#include <ros/ros.h>
#include <modules_map_msgs/LineSegment.h>
#include <modules_map_msgs/BoundaryEdge.h>
#include <modules_map_msgs/RoadROIBoundary.h>
#include <modules_map_msgs/JunctionOverlapInfo.h>
#include <modules_map_msgs/Junction.h>
#include <modules_map_msgs/BoundaryEdgeType.h>
...
// HDMapInput ROS
HDMapInput::HDMapInput(ros::NodeHandle& nh) :
  nh_(nh),
  map_dir("map/data/demo"),
  base_map_filename("base_map.xml|base_map.bin|base_map.txt"),
  sim_map_filename("sim_map.bin|sim_map.txt"),
  routing_map_filename("routing_map.bin|routing_map.txt"),
  end_way_point_filename("default_end_way_point.txt"),
  vehicle_config_path("common/data/mkz_config.pb.txt"),
  use_ros_time(false),
  map_radius(60.0),
  map_sample_step(1){}
...
bool HDMapInput::GetROI(const PointD& pointd, HdmapStructPtr* mapptr) {
  ...
  modules_common_msgs::PointENU point;
...
int HDMapInput::MergeBoundaryJunction(
  ...
  for (size_t i = 0; i < boundaries.size(); i++) {
    for (const auto& road_boundary : boundaries[i]->road_boundaries) {
      ...
      for (const modules_map_msgs::BoundaryEdge& edge : road_boundary.outer_polygon.edge) {
        PolygonDType* edge_side = nullptr;
        if (edge.type == modules_map_msgs::BoundaryEdgeType::LEFT_BOUNDARY) {
          edge_side = &mapped_boundary.left_boundary;
        } else if (edge.type == modules_map_msgs::BoundaryEdgeType::RIGHT_BOUNDARY) {
          edge_side = &mapped_boundary.right_boundary;
        } else {
          continue;
        }
        for (const modules_map_msgs::CurveSegment& segment : edge.curve.segment) {
          if (!segment.line_segment.empty()) {
	    modules_map_msgs::LineSegment* line_segment_ = nullptr;
	    line_segment_->point = segment.line_segment.data()->point;
            DownSampleBoundary(line_segment_, edge_side);
          }
        }
      }
    }
