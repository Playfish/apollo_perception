...
#include "perception/obstacle/lidar/interface/base_roi_filter.h"
...
#include "perception/obstacle/lidar/roi_filter/hdmap_roi_filter/bitmap2d.h"
#include "perception/obstacle/lidar/roi_filter/hdmap_roi_filter/polygon_mask.h"
#include "perception/obstacle/lidar/roi_filter/hdmap_roi_filter/polygon_scan_converter.h"

//#include "perception/obstacle/onboard/hdmap_input.h"
...
class HdmapROIFilter : public BaseROIFilter {
 public:
  HdmapROIFilter(ros::NodeHandle& nh);
  ...
  /**
   * @brief 以世界地图中的ROI为模板，去除当前velodyne点云中的背景，如路边建筑物和树木等,
   * @params[In] cloud: All the cloud points with local coordinates,激光雷达捕捉的3D点数据集
   * @params[In] roi_filter_options: Type definition in, 多边形集合，每个多边形均含有一个有序的点集。
   * "../../interface/base_roi_filter.h". Contains the information
   * of ROI and world to local coordinates transformation matrix.
   * @params[Out] roi_indices: The indices of points within ROI, 由高精地图定义的ROI内的输入点索引
